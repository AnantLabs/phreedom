...
class nova_xml {
  ...
  function keys() {
    return array(
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_MERCHANT_ID',       'default' => ''),
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_USER_ID',           'default' => ''),
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_PIN',               'default' => ''),
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_TESTMODE',          'default' => 'Production'),
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_AUTHORIZATION_TYPE','default' => 'Authorize/Capture'),
	  array('key' => 'MODULE_PAYMENT_NOVA_XML_SORT_ORDER',        'default' => '2'),
	);
  }
  ...
  function javascript_validation() {
    ...
    '      error_message = error_message + "' . MODULE_PAYMENT_NOVA_XML_TEXT_JS_CC_OWNER . '";' . "\n" .
    '      error = 1;' . "\n" .
    '    }' . "\n" .
    '    if (cc_number == "" || cc_number.length < ' . CC_NUMBER_MIN_LENGTH . ') {' . "\n" .
    '      error_message = error_message + "' . MODULE_PAYMENT_NOVA_XML_TEXT_JS_CC_NUMBER . '";' . "\n" .
    '      error = 1;' . "\n" .
    '    }' . "\n" . 
    '    if (cc_cvv == "" || cc_cvv.length < "3" || cc_cvv.length > "4") {' . "\n".
    '      error_message = error_message + "' . MODULE_PAYMENT_NOVA_XML_TEXT_JS_CC_CVV . '";' . "\n" .
    '      error = 1;' . "\n" .
    '    }' . "\n" . 
    '  }' . "\n";
    return $js;
  ...
  function selection() {
	...
	$selection = array(
	  'id' => $this->code,
	  'page' => MODULE_PAYMENT_NOVA_XML_TEXT_CATALOG_TITLE,
	  'fields' => array(
	    array(
		  'title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_OWNER,
		  'field' => html_input_field('nova_xml_field_0', $order->nova_xml_field_0)),
		array(
		  'title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_NUMBER,
		  'field' => html_input_field('nova_xml_field_1', $order->nova_xml_field_1)),
		array(
		  'title' => MODULE_PAYMENT_NOVA_XML_TEXT_CREDIT_CARD_EXPIRES,
		...
		array(
		  'title' => MODULE_PAYMENT_NOVA_XML_TEXT_CVV,
		  'field' => html_input_field('nova_xml_field_4', $order->nova_xml_field_4, 'size="4" maxlength="4"'))
		),
	);
    return $selection;
  }
  ...
  function pre_confirmation_check() {
	...
	if (strpos($_POST['nova_xml_field_1'],'*') !== false) {
    	$messageStack->add(MODULE_PAYMENT_NOVA_XML_NO_DUPS, 'caution');
		return false;
	}
  ...
  function before_process() {
    ...
    $order->info['cc_expires'] = $_POST['nova_xml_field_2'] . $_POST['nova_xml_field_3'];
...
*/
    ...
    if ($results['errorCode']) {
      $messageStack->add('Decline Code #' . $results['errorCode'] . ': ' . $results['errorMessage'] . ' - ' . MODULE_PAYMENT_NOVA_XML_TEXT_DECLINED_MESSAGE, 'error');
	  return true;
	...
	$messageStack->add($results['ssl_result_message'] . ' - ' . MODULE_PAYMENT_NOVA_XML_TEXT_DECLINED_MESSAGE, 'error');
	return true;
  }
}
