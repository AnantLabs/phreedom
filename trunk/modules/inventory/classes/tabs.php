...
class tabs {
  ...
  function btn_save($id = '') {
    ...
    if ($id) {
	  ...
	  // Test for duplicates.
	  $result = $db->Execute("select id from " . TABLE_EXTRA_TABS . " where module_id='inventory' and tab_name='" . $tab_name . "'");
  ...
  function build_main_html() {
	...
	$heading_array = array(
	  'tab_name'    => INV_HEADING_CATEGORY_NAME,
    ...
    $output    .= '  </tr>' . chr(10);
	// Build field data
    $query_raw  = "select id, tab_name, description, sort_order from " . TABLE_EXTRA_TABS . " where module_id='inventory' order by $disp_order";
    ...
    while (!$result->EOF) {
	  ...
	  if ($_SESSION['admin_security'][SECURITY_ID_CONFIGURATION] > 3) $output .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . EXTRA_TABS_DELETE_INTRO . '\')) tabPage(\'tabs\', \'delete\', ' . $result->fields['id'] . ')"') . chr(10);
      $output .= '    </td>' . chr(10);
      $output .= '  </tr>' . chr(10);
      $result->MoveNext();
    }
  ...
  function build_form_html($action, $id = '') {
	...
	$output .= '    <td colspan="2">' . ($action=='new' ? EXTRA_TABS_INSERT_INTRO : INV_EDIT_INTRO) . '</td>' . chr(10);
    $output .= '  </tr>' . chr(10);
	$output .= '  <tr class="dataTableRow">' . chr(10);
