...
// Build content box
         $sql = "select audit_log.action_date, audit_log.action, audit_log.reference_id, audit_log.amount, users.display_name from audit_log, users where audit_log.user_id = users.admin_id and audit_log.action_date >= '" . date('Y-m-d', strtotime('-' . $params['today_minus'] . ' day')) . "'  and audit_log.action_date <= '" . date('Y-m-d', strtotime('-' . $params['today_minus'] + 1 . ' day')) . "' order by audit_log.action_date desc";
