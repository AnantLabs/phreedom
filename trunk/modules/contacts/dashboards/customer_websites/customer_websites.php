...
define('DASHBOARD_CUSTOMER_WEBSITES_VERSION','3.2');

class customer_websites extends ctl_panel {
  function customer_websites() {
  }
  ...
  function Output($params) {
	...
	if ($result->RecordCount() < 1) {
	  $contents = CP_CUSTOMER_WEBSITES_NO_RESULTS;
	} else {
	  while (!$result->EOF) {
	...
	return $this->build_div(CP_CUSTOMER_WEBSITES_TITLE, $contents, $control);
  }

  function Update() {
	global $db;
	$my_title  = db_prepare_input($_POST['customer_websites_field_0']);
	$my_url    = db_prepare_input($_POST['customer_websites_field_1']);
