...
class tax_auths {
  ...
  function build_main_html() {
	...
	while (!$result->EOF) {
	  $actions = '';
	  if ($this->security_id > 1) $actions .= html_icon('actions/edit-find-replace.png', TEXT_EDIT, 'small', 'onclick="loadPopUp(\''.get_called_class().'_edit\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
	  if ($this->security_id > 3) $actions .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SETUP_TAX_AUTH_DELETE_INTRO . '\')) subjectDelete(\''.get_called_class().'\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
    ...
    return html_datatable(''.get_called_class().'_'.$this->type.'_table', $content);
  }
