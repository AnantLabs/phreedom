...
/**************************************************************************************************************/
  ...
  function html_heading_bar($heading_array, $sort_field = '', $sort_order = 'asc', $extra_headings = array(TEXT_ACTION)) {
	global $PHP_SELF; 
	$result = array();
	$output .= html_hidden_field('sort_field', $_POST['sort_field']) . chr(10);
    $output .= html_hidden_field('sort_order', $_POST['sort_order']) . chr(10);
	foreach ($heading_array as $key => $value) {
	  ...
	  if ($value) $output .= html_image(DIR_WS_IMAGES . $image_asc  , TEXT_ASC,  $width = '', $height = '', $params = 'onclick="submitSortOrder(\''.$value.'\',\'asc\')"'). chr(10); // . '</a>' . chr(10);
	  $output .= $value;
	  if ($value) $output .= html_image(DIR_WS_IMAGES . $image_desc , TEXT_DESC, $width = '', $height = '', $params = 'onclick="submitSortOrder(\''.$value.'\',\'desc\')"'). chr(10); // . '</a>' . chr(10);
	  $output .= '</th>' . chr(10);
	}
