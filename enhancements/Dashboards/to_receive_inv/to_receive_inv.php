...
define('DASHBOARD_TO_RECEIVE_INV_VERSION','3.3');
...
class to_receive_inv extends ctl_panel {
  ...
  function Output($params) {
	...
	if ($result->RecordCount() < 1) {
	  $contents = CP_TO_RECEIVE_INV_NO_RESULTS;
	} else {
	  while (!$result->EOF) {
	...
	return $this->build_div(CP_TO_RECEIVE_INV_TITLE, $contents, $control);
  }

  function Update() {
	global $db;
	$params['num_rows'] = db_prepare_input($_POST['to_receive_inv_field_0']);
