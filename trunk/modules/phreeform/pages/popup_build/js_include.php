...
<?php 
  if ($report->reporttype == 'frm') {
    ...
    echo "  tableInit[tableCount] = 'field_setup_frm'; tableCount++;" . chr(10);
  } else if ($report->reporttype == 'rpt') {
    echo "  tableInit[tableCount] = 'table_setup';     tableCount++;" . chr(10);
    echo "  tableInit[tableCount] = 'sort_setup';      tableCount++;" . chr(10);
    echo "  tableInit[tableCount] = 'crit_setup';      tableCount++;" . chr(10);
    echo "  tableInit[tableCount] = 'field_setup';     tableCount++;" . chr(10);
    echo "  tableInit[tableCount] = 'group_setup';     tableCount++;" . chr(10);
  } 
?>
...
//alert('table = '+idTable+' and rCLick = '+rClick+' and action = '+action+' box id = '+boxID);
  switch (action) {
	case 'add':
	  ...
	  break;
	case 'delete':
	  if (boxID) idTable += boxID;
	  var oTable = document.getElementById(idTable);
	  oTable.deleteRow(rClick);
	  break;
  }
  return;
}
...
//alert('idTable = '+idTable+' and rIndex = '+rIndex+' and boxID = '+boxID+' and rowCnt = '+rowCnt);
  ...
  switch (idTable) {
    ...
    case 'group_setup':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'group_setup\', \'delete\')"')); ?>';
	...
	case 'sort_setup':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'sort_setup\', \'delete\')"')); ?>';
	  cell[3]  = temp.replace(/row_TBD/g, rowCnt);
	  attr[3]  = 'right';
      break;
	case 'crit_setup':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'crit_setup\', \'delete\')"')); ?>';
	...
	case 'field_setup':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'field_setup\', \'delete\'); calculateWidth();"')); ?>';
	...
	case 'box_Tbl':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'box_Tbl\', \'delete\', row_TBD)"')); ?>';
    ...
    case 'box_Ttl':
	  ...
	  temp    += '<?php echo str_replace("'", "\'", html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . TEXT_DELETE_ENTRY . '\')) rowAction(\'box_Ttl\', \'delete\', row_TBD)"')); ?>';
	  cell[2]  = temp.replace(/row_TBD/g, boxID);
	  attr[2]  = 'right';
 	  for (var i=0; i<cell.length; i++) cell[i] = cell[i].replace(/boxTBD/g, boxID);
      break;
