...
// Build the page
?>
<div class="pageHeading"><?php echo constant('ORD_TEXT_' . JOURNAL_ID . '_' . strtoupper($type) . '_WINDOW_TITLE'); ?></div>
<div>
...
?>
	  ...
	  <?php if (ENABLE_MULTI_BRANCH) { ?>
		  <tr>
		    <td class="main" align="right"><?php echo GEN_STORE_ID . '&nbsp;'; ?></td>
			<td class="main" align="right"><?php echo html_pull_down_menu('store_id', gen_get_store_ids(), $order->store_id ? $order->store_id : $_SESSION['admin_prefs']['def_store_id']); ?></td>
		  </tr>
	  <?php } ?>
		  <tr>
		  ...
		  <tr>
            <td class="main" align="right"><?php echo JOURNAL_ID==20 ? TEXT_BUYER : TEXT_SALES_REP; ?></td>
            <td class="main" align="right"><?php echo html_pull_down_menu('rep_id', gen_get_rep_ids($account_type), $order->rep_id ? $order->rep_id : $default_sales_rep); ?></td>
		  </tr>
		  <tr>
  ...
  <table id="item_table" width="100%" border="1" cellpadding="1" cellspacing="1">
	...
	<?php if ($order->id_1) {
	  ...
	  while (true) {
		...
		// End hidden fields
		echo '</td>' . chr(10);
		echo '<td class="main" align="center">' . html_input_field($due_num,   gen_locale_date($order->$due_num), 'readonly="readonly" size="15"') . '</td>' . chr(10);
