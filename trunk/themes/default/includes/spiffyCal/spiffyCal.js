...
// GLOBAL variables
var scImgPath = 'themes/default/includes/spiffyCal/images/';
...
'MMM/dd/yyyy'
	...
	function validateDate(eInput, bRequired, dStartDate, dEndDate ,InputValue, formName, textBoxName){
		...
		if( InputValue == null ){
			InputValue = eInput.value;	
		}else{
			eInput     = document[formName][textBoxName];
		}
		this.matchedFormat = "";
		bIsEmpty = ( InputValue == '' || InputValue == null);
		if (!(bRequired && bIsEmpty)) {
			for( i = 0; i < this.AllowedFormats.length; i++){
				if (isDate(InputValue, this.AllowedFormats[i]) == true){
					bOK = true;
					formatMatchCount += 1;
		...
		if (formatMatchCount>1) {
			if (this.showHelpAlerts) {
				var date1 = getDateFromFormat( InputValue, this.AllowedFormats[firstMatchAt] );
				var date2 = getDateFromFormat( InputValue, this.AllowedFormats[secondMatchAt] );
		...
		if ( bOK == true ) {
			...
			if ( dStartDate != null && dEndDate != null ) {
				...
				var rangeMsg = 'This input box is set up to accept dates from:<br>   '+
						strStart + ' and ' + strEnd + '<br>Please enter a date not';
				var finshMsg = '<br>Or contact your adminstrator';

				var dThis = getDateFromFormat( InputValue, this.AllowedFormats[firstMatchAt] );
				if ( dStartDate > dThis ){
					eInput.className = "cal-TextBoxInvalid"; 
					$("#date_error_text").html(rangeMsg + 'earlier than  '+ strStart + finshMsg);
					$("#date_error").show() ;
					return false;
				}
				if ( dEndDate <= dThis ) {
					eInput.className = "cal-TextBoxInvalid";
					$("#date_error_text").html(rangeMsg + 'later than  '+ strEnd + finshMsg);
					$("#date_error").show() ;
					return false;
				}
				...
				if ( firstDayMonth > dThis || firstDayNextMonth <= dThis ) {
					$("#out_of_period").show();
				}
			}
		...
		//	this.lastBoxValidated = null;
			...
			if (bRequired && bIsEmpty) {
				eInput.className = "cal-TextBoxInvalid";
				$("#date_error_text").html('This date field is required.<br>Please enter a valid date before proceeding.');
				$("#date_error").show() ;
			}else {
				if (!bRequired && bIsEmpty) {
					eInput.className = "cal-TextBox";
					...
					$("#date_error_text").html('Please enter a valid date.<br>Example 01-Jan-2002<br>Valid formats are:<br>'+strTemp);
					$("#date_error").show() ;
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	var msNames     = new makeArray0(month_short_01,month_short_02,month_short_03,month_short_04,month_short_05,month_short_06,month_short_07,month_short_08,month_short_09,month_short_10,month_short_11,month_short_12);
	var msDays      = new makeArray0(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
	var msDOW       = new makeArray0(day_short_1,day_short_2,day_short_3,day_short_4,day_short_5,day_short_6,day_short_7);
	...
	// Defaults
	var strDefDateFmt = this.defaultDateFormat;//'dd-MMM-yyyy';
...
// mod per phreesoft for multiple buttons
    this.btnName=this.varName + '-' + strBtnName;
//    this.btnName=strBtnName;
	...
	this.minYearChoice=dteToday.getFullYear()-5;
...
//		eval('document.'+this.formName+'.'+this.textBoxName).value=strWhat;
	...
	function disable() {
		...
		document[this.formName][this.textBoxName].disabled = true;
        //eval('document.'+this.formName+'.'+this.textBoxName).className = "cal-TextBoxDisabled";
        document[this.formName][this.textBoxName].className = "cal-TextBoxDisabled";
		if (scNN) {
			//eval('document.'+this.formName+'.'+this.textBoxName).onFocus= function() {this.blur();};
			document[this.formName][this.textBoxName].onFocus = function() {this.blur();};
		}
	}
	...
	function enable() {
		...
		document[this.formName][this.textBoxName].disabled = false;
		//eval('document.'+this.formName+'.'+this.textBoxName).disabled=false;
		document[this.formName][this.textBoxName].className = "cal-TextBox";
        //eval('document.'+this.formName+'.'+this.textBoxName).className = "cal-TextBox";
		if (scNN) {
			document[this.formName][this.textBoxName].onFocus = null;
...
// put these next 2 lines in when the tiny call buttons seem to randomly disappear
			...
			if ( this.focusClick == true && this.hideButton == true ) {
				//if no drop down button then use user-provided location for it
				scX=this.x;
				scY=this.y;
			} else {
...
//			eval('document.'+this.formName+'.'+this.textBoxName).className = "cal-TextBoxInvalid";
	...
	function changeDay(intWhatDay) {
		...
		//eval('document.'+this.formName+'.'+this.textBoxName).className = "cal-TextBox";
		document[this.formName][this.textBoxName].className = "cal-TextBox";
		calMgr.validateDate('', this.required, this.minDate, this.maxDate, calMgr.scFormatDate(dteCur,this.dateFormat), this.formName, this.textBoxName);
	}
	...
	function isDayValid(intWhatDay, intWhatMonth, intWhatYear){
		...
		dteCur.setFullYear(intWhatYear);
		dteCur.setHours(0);dteCur.setMinutes(0);dteCur.setSeconds(0);dteCur.setMilliseconds(0);
		this.minDate.setHours(0);this.minDate.setMinutes(0);this.minDate.setSeconds(0);this.minDate.setMilliseconds(0);
