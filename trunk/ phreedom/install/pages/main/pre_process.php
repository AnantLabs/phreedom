...
switch ($action) {
  ...
  case 'inspect':
    // check for decline
	if ($_POST['license'] == 'disagree') { header('location: http://www.google.com'); }
...
//echo 'server = '; print_r($_SERVER); echo '<br>';
	  ...
	  foreach ($core_modules as $entry) {
		if ($entry <> '.' && $entry <> '..' && is_dir(DIR_FS_MODULES . $entry)) {
		  if (file_exists(DIR_FS_MODULES . $entry . '/config.php')) {
		    ...
		    if (admin_check_versions($entry, $install_mod->prerequisites)) { // Check for version levels
			  $error = true;
	    ...
	    foreach ($contents as $entry) { // install each module
		  ...
		  if ($entry <> '.' && $entry <> '..' && is_dir(DIR_FS_MODULES . $entry)) {
		    if (file_exists(DIR_FS_MODULES . $entry . '/config.php')) {
		      ...
		      if (admin_check_versions($entry, $install_mod->prerequisites)) { // Check for version levels
			    $error = true;
		      } elseif (admin_install_dirs($install_mod->dirlist, DIR_FS_MY_FILES.$_SESSION['company'].'/')) { // Create any new directories
			    $error = true;
		      } elseif (admin_install_tables($install_mod->tables)) { // Create the tables
			    $error = true; 
		      } else {
	...
	if (!$error) { // input admin username record, clear the tables first
	  ...
	  if (sizeof($params) > 0) { // create My Notes dashboard entries
