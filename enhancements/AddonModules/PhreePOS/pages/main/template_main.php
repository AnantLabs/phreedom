...
// print previous receipt
  ...
  $toolbar->icon_list['previous_print']['text'] = 'Print Previous';
// open drawer if code is pressent
  if (defined('PHREEPOS_RECEIPT_PRINTER_OPEN_DRAWER') && PHREEPOS_RECEIPT_PRINTER_OPEN_DRAWER <> '') {  
  	...
  	$toolbar->icon_list['open_drawer']['text'] = 'Open Drawer';
  }
...
<div id="inv_popup" title="last item added">
    <?php echo html_image('', '', '', '600');?>
</div>
<div id="inv_image">
    <?php echo html_image('', '', '', '100', 'onclick="showImage()" id="curr_image"');   ?>
</div>
  ...
  <tr>
    <td valign="top">
      ...
      <?php if ($tills->showDropDown()) {	// show currency slection pulldown 
    			echo TEXT_TILL . ' ' . html_pull_down_menu('till_id', $tills->till_array() , $order->till_id ? $order->till_id : $tills->default_till()); 
			}else{ 
        ...
        <tr>
          <td>
  		<?php echo TEXT_SALES_REP . ' ' . html_pull_down_menu('rep_id', gen_get_rep_ids($account_type), $order->rep_id ? $order->rep_id : $default_sales_rep); ?>
          </td>
        </tr>
<?php if (ENABLE_MULTI_CURRENCY) {	// show currency slection pulldown ?>
		<tr>
		  <td>
		    <?php echo TEXT_CURRENCY . ' ' . html_pull_down_menu('display_currency', gen_get_pull_down(TABLE_CURRENCIES, false, false, 'code', 'title'), $order->currencies_code, 'onchange="recalculateCurrencies();"'); ?>
			<?php echo TEXT_EXCHANGE_RATE . ' ' . html_input_field('currencies_value', $order->currencies_value, 'readonly="readonly"'); ?>
		  </td>
		</tr>
<?php } ?>
		<tr id="search_part">
		  <td>
<?php
  echo TEXT_SKU . ' ' . html_input_field('sku', '', ' size="' . (MAX_INVENTORY_SKU_LENGTH + 1) . '" maxlength="' . MAX_INVENTORY_SKU_LENGTH . '" onfocus="clearField(\'sku\', \'' . TEXT_SEARCH . '\')" onblur="setField(\'sku\', \'' . TEXT_SEARCH . '\');" onChange ="loadSkuDetails(0, 0)"') . chr(10);
...
<!-- Column 2 - Customers and Payment -->
    <td>
	  <table>
		<tr>
		  <td>
<?php 
  echo ORD_ACCT_ID . ' ' . html_input_field('search', isset($order->short_name) ? $order->short_name : TEXT_SEARCH, 'size="21" maxlength="20" onfocus="clearField(\'search\', \'' . TEXT_SEARCH . '\')" onblur="setField(\'search\', \'' . TEXT_SEARCH . '\');"');
...
<?php
  ...
  echo html_input_field('bill_contact',     $order->bill_contact, 'size="33" maxlength="32" onfocus="clearField(\'bill_contact\', \'' . GEN_CONTACT . '\')" onblur="setField(\'bill_contact\', \'' . GEN_CONTACT . '\')"', ADDRESS_BOOK_CONTACT_REQUIRED) . '<br />' . chr(10);
  ...
  echo html_pull_down_menu('bill_country_code', gen_get_countries(), $order->bill_country_code ? $order->bill_country_code : COMPANY_COUNTRY) . '<br />' . chr(10); 
  ...
  ?>
		...
		<tr>
          <td align="right">
<?php echo TEXT_SUBTOTAL . ' ' . html_input_field('subtotal', $currencies->format($order->subtotal, true, $order->currencies_code, $order->currencies_value), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		  </td>
		</tr>
		<tr>
...
?>
          <td align="right">
<?php echo TEXT_DISCOUNT_PERCENT . ' ' . html_input_field('disc_percent', ($order->disc_percent ? number_format(100*$order->disc_percent,3) : '0'), 'size="7" maxlength="6" onchange="calculateDiscountPercent()" style="text-align:right"') . ' '; ?>
<?php echo TEXT_AMOUNT . ' ' . html_input_field('discount', $currencies->format(($order->discount ? $order->discount : '0'), true, $order->currencies_code, $order->currencies_value), 'size="15" maxlength="20" onchange="calculateDiscount()" style="text-align:right"'); ?>
		  </td>
		</tr>
		<tr>
<?php } else {
  ...
  $hidden_fields .= html_hidden_field('disc_percent', '0') . chr(10);
          ...
          <td align="right">
<?php echo ORD_SALES_TAX . ' ' . html_input_field('sales_tax', $currencies->format(($order->sales_tax ? $order->sales_tax : '0.00'), true, $order->currencies_code, $order->currencies_value), 'readonly="readonly" size="15" maxlength="20" onchange="updateTotalPrices()" style="text-align:right"'); ?>
		  </td>
		</tr>
<?php if(!PHREEPOS_ROUNDING == 0) {?>		
		<tr>
          <td align="right">
<?php echo TEXT_ROUNDING_OF . ' ' . html_input_field('rounded_of', $currencies->format($order->rounded_of, true, $order->currencies_code, $order->currencies_value), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		  </td>
		</tr>
<?php }?>
		<tr>
...
<?php echo TEXT_TOTAL . ' ' . html_input_field('total', $currencies->format($order->total_amount, true, $order->currencies_code, $order->currencies_value), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		...
		<tr>
          <td align="right">
<?php echo TEXT_AMOUNT_PAID . ' ' . html_input_field('pmt_recvd', $currencies->format($order->pmt_recvd), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		  </td>
		</tr>
		<tr>
          <td align="right">
<?php echo TEXT_BALANCE_DUE . ' ' . html_input_field('bal_due', $currencies->format($order->bal_due), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		  </td>
		</tr>
		<tr>
		  <td><?php echo html_button_field('payment', TEXT_PAYMENT, 'onclick="centerPopup(), popupPayment()"'); ?>
		  </td>
		</tr>
		<tr>
		  <td>
...
if ($order->pmt_rows) {
  ...
  <tr>
    <td><?php echo "<b><u>" . TEXT_NOTES . "</u></b><br>" . PHREEPOS_ITEM_NOTES; ?></td>
    <td><?php echo '&nbsp;'; ?></td>
  </tr>
</table>
...
?>
  <applet name="jZebra" code="jzebra.RawPrintApplet.class" archive="<?php echo DIR_WS_ADMIN . 'modules/phreedom/includes/jzebra/jzebra.jar'; ?>" width="16" height="16">
    <param name="printer" value="<?php echo PHREEPOS_RECEIPT_PRINTER_NAME; ?>">
  </applet>
