...
$period = $_POST['search_period'] ? $_POST['search_period'] : CURRENT_ACCOUNTING_PERIOD;
if ($period == 'all') {
...
$sql = "select i.id, m.post_date, i.debit_amount, i.credit_amount, m.purchase_invoice_id, m.bill_primary_name, i.description, m.journal_id 
	...
	where i.gl_account = '" . $gl_account . "' 
	  and (i.reconciled = 0 or i.reconciled > $period) and m.post_date <= '" . $fiscal_dates['end_date'] . "'";
$result = $db->Execute($sql);
while (!$result->EOF) {
