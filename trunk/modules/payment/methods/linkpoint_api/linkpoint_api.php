...
//		$this->sort_order  = MODULE_PAYMENT_LINKPOINT_API_SORT_ORDER; // Sort Order of this payment option on the customer payment page
  ...
  function keys() {
    return array(
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_LOGIN',                    'default' => 'YourStoreNumber'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE_RESPONSE','default' => 'LIVE: Production'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_AUTHORIZATION_MODE',       'default' => 'Immediate Charge/Capture'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_FRAUD_ALERT',              'default' => '1'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_STORE_DATA',               'default' => '1'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_TRANSACTION_MODE',         'default' => 'Production'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_DEBUG',                    'default' => 'Off'),
	  array('key' => 'MODULE_PAYMENT_LINKPOINT_API_SORT_ORDER',               'default' => '8'),
	);
  }
		...
		$selection = array (
			...
			'fields' => array (
				...
				array (
					...
					'field' => html_input_field('linkpoint_api_cc_cvv', '', 'size="4" maxlength="4"' . ' id="' . $this->code . '-cc-cvv"' ) . ' ' . '<a href="javascript:popupWindow(\'' . html_href_link(FILENAME_POPUP_CVV_HELP) . '\')">' . MODULE_PAYMENT_LINKPOINT_API_TEXT_POPUP_CVV_LINK . '</a>',
...
*/
		...
		if ($result["r_approved"] != "APPROVED") {
			...
			if (substr($result['r_error'], 0, 10) == 'SGS-000001' || strstr($result['r_error'], 'D:Declined') || strstr($result['r_error'], 'R:Referral')) {
				//$messageStack->add_session( MODULE_PAYMENT_LINKPOINT_API_TEXT_DECLINED_MESSAGE . '<br />' . $result['r_error'], 'error');
				$messageStack->add( MODULE_PAYMENT_LINKPOINT_API_TEXT_DECLINED_MESSAGE . '<br />' . $result['r_error'], 'error');
			}
		...
		if ($result["r_approved"] != "APPROVED") {
			// alert to customer:
			$messageStack->add(MODULE_PAYMENT_LINKPOINT_API_TEXT_DECLINED_MESSAGE, 'caution');
