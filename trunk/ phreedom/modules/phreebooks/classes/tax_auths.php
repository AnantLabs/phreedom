...
class tax_auths {
  ...
  function build_main_html() {
    ...
    while (!$result->EOF) {
      ...
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')">' . htmlspecialchars($result->fields['description_short']) . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')">' . htmlspecialchars($result->fields['description_long']) . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')">' . gen_get_type_description(TABLE_CHART_OF_ACCOUNTS, $result->fields['account_id']) . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')">' . $result->fields['tax_rate'] . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" align="right">' . chr(10);
	  if ($this->security_id > 1) $output .= html_icon('actions/edit-find-replace.png', TEXT_EDIT, 'small', 'onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
	  if ($this->security_id > 3) $output .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SETUP_TAX_AUTH_DELETE_INTRO . '\')) subjectDelete(\'tax_auths\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
      $output .= '    </td>' . chr(10);
      $output .= '  </tr>' . chr(10);
      $result->MoveNext();
    }
    $output .= '</table>' . chr(10);
    $output .= '<div class="page_count_right">' . $page_split->display_ajax($query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['list'], '', 'tax_auths_list', 'tax_auths') . '</div>' . chr(10);
