...
function build_groups() { // dynamically build report and form groups
  ...
  while (!$result->EOF) {
    if ($result->fields['doc_ext'] == 'ff') {
	  $output['forms'][$result->fields['doc_group']]   = $result->fields['doc_title'];
	} else { // else its a report
	  $output['reports'][$result->fields['doc_group']] = $result->fields['doc_title'];
	}
	$result->MoveNext();
  }
  return $output;
