...
<?php if (ENABLE_MULTI_BRANCH) {	// show currency slection pulldown ?>
		<tr>
		  <td>
    		<?php echo TEXT_BRANCH . ' ' . html_pull_down_menu('store_id', gen_get_store_ids(), $order->store_id ? $order->store_id : $_SESSION['admin_prefs']['def_store_id']); ?>
		  </td>
		</tr>
<?php } ?>
...
<?php } ?>
		<tr>
		  <td>
<?php
  echo TEXT_SKU . ' ' . html_input_field('sku', '', ' size="' . (MAX_INVENTORY_SKU_LENGTH + 1) . '" maxlength="' . MAX_INVENTORY_SKU_LENGTH . '" onfocus="clearField(\'sku\', \'' . TEXT_SEARCH . '\')" onblur="setField(\'sku\', \'' . TEXT_SEARCH . '\'); loadSkuDetails(0, 0)"') . chr(10);
...
<?php echo TEXT_BALANCE_DUE . ' ' . html_input_field('bal_due', $currencies->format($order->bal_due), 'readonly="readonly" size="15" maxlength="20" style="text-align:right"'); ?>
		...
		<tr>
		  <td><?php echo html_button_field('payment', TEXT_REFUND, 'onclick="centerPopup(), popupPayment()"'); ?>
		  </td>
		</tr>
		<tr>
		  <td>
...
?>
	...
	<tr>
	  ...
	  <td><?php echo html_icon('phreebooks/pdficon_large.gif', TEXT_PRINT, 'large', 'onclick="SavePayment(\'print_return\')"', 0, 0, 'btn_save'); ?></td>
	</tr>
