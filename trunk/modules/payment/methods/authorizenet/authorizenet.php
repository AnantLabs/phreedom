...
class authorizenet {
  ...
  function keys() {
    return array(
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_LOGIN',              'default' => ''),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_TXNKEY',             'default' => 'Test'),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_MD5HASH',            'default' => '*Set A Hash Value at AuthNet Admin*'),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_TESTMODE',           'default' => 'Test'),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_AUTHORIZATION_TYPE', 'default' => 'Capture'),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_USE_CVV',            'default' => '1'),
	  array('key' => 'MODULE_PAYMENT_AUTHORIZENET_SORT_ORDER',         'default' => '5'),
	);
  }
  ...
  function selection() {
    ...
    $selection = array(
	  'id'     => $this->code,
	  'page' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CATALOG_TITLE,
	  'fields' => array(
        ...
        array('title' => MODULE_PAYMENT_AUTHORIZENET_TEXT_CVV,
			  'field' => html_input_field('authorizenet_field_4', $order->authorizenet_field_4)),
	  ));
  ...
  function pre_confirmation_check() {
	...
	if (strpos($_POST['authorizenet_field_1'],'*') !== false) {
    	$messageStack->add(MODULE_PAYMENT_AUTHORIZENET_NO_DUPS, 'caution');
		return false;
	}
...
//echo 'submit_data = '; print_r($submit_data); echo '<br>';
    ...
    if ($response_code != '1') {
      $messageStack->add($response_to_customer . ' - ' . MODULE_PAYMENT_AUTHORIZENET_TEXT_DECLINED_MESSAGE, 'error');
      return true;
    }
    if ($response[88] != '') {
