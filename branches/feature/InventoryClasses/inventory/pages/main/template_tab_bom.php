...
<?php
	...
	if (count($cInfo->bom_list)) {
		for ($j = 0, $i = 1; $j < count($cInfo->bom_list); $j++, $i++) {
			$readonly = '';
			echo '    <tr>';
			echo '      <td>';
			...
			echo '      <td>';
			// Hidden fields
			echo '      	<input type="hidden" name="id_' . $i . '" id="id_' . $i . '" value="' . $cInfo->bom_list[$j]['id'] . '" />' . chr(10);
			// End hidden fields
			echo '			<input type="text" name="assy_sku[]" id="sku_' . $i . '" value="' . $cInfo->bom_list[$j]['sku'] . '" ' . $readonly . 'size="' . (MAX_INVENTORY_SKU_LENGTH + 1) . '" maxlength="' . MAX_INVENTORY_SKU_LENGTH . '" />&nbsp;' . chr(10);
			...
			echo '      </td>' . chr(10);
			echo '      <td><input type="text" name="assy_desc[]" 			id="desc_' . $i . '" 		value="' . $cInfo->bom_list[$j]['description'] . '" ' . $readonly . 'size="64" maxlength="64" /></td>' . chr(10);
			echo '      <td><input type="text" name="assy_qty[]" 			id="qty_' . $i . '" 		value="' . $cInfo->bom_list[$j]['qty'] . '" 		' . $readonly . 'size="6" maxlength="5" /></td>' . chr(10);
			echo '      <td><input type="text" name="assy_item_cost[]" 		id="item_cost_' . $i . '" 	value="' . $currencies->precise($cInfo->bom_list[$j]['item_cost']) . '" 	' . $readonly . 'size="6" maxlength="5" /></td>' . chr(10);
			echo '      <td><input type="text" name="assy_sales_price[]" 	id="sales_price_' . $i . '" value="' . $currencies->precise($cInfo->bom_list[$j]['full_price']) . '" 	' . $readonly . 'size="6" maxlength="5" /></td>' . chr(10);
			echo '    </tr>';
		}
	} else {
...
<?php if ($cInfo->allow_edit_bom == false) echo '<script language="JavaScript">bomTotalValues();</script>';?>