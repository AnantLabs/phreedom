...
function removeBOMRow(delRowCnt) {
  ...
  for (var i=delRowCnt; i<(document.getElementById("bom_table").rows.length); i++) {
	...
	document.inventory.elements['item_cost_'+i].value  	= document.inventory.elements['item_cost_'+(i+1)].value;
	document.inventory.elements['sales_price_'+i].value = document.inventory.elements['sales_price_'+(i+1)].value;
// Hidden fields
...
function masterStockTitle(id) {
  if(document.all) { // IE browsers
    document.getElementById('sku_list').rows[1].cells[id+2].innerText = document.getElementById('attr_name_'+id).value;
  } else { //firefox
    document.getElementById('sku_list').rows[1].cells[id+2].textContent = document.getElementById('attr_name_'+id).value;
  }
}
