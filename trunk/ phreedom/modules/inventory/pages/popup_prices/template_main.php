...
// Build the page
?>
<div class="pageHeading"><?php echo PAGE_TITLE . ' - ' . $sku; ?></div>
...
<?php if ($type == 'v') { ?>
  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="setReturnPrice('<?php echo $rowId; ?>', 'cost');">
...
<?php } else { ?> 
  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="setReturnPrice('<?php echo $rowId; ?>', 'full');">
    <td><?php echo INV_ENTRY_FULL_PRICE . ': '; ?></td>
    <td><?php echo html_input_field('full', $currencies->precise($inventory_details->fields['full_price']), 'readonly="readonly" size="10" style="text-align:right"'); ?></td>
  </tr>
<?php } ?> 
</table>
<?php
if (is_object($price_sheets)) {
  if ($price_sheets->RecordCount() > 0) {
	...
	while (!$price_sheets->EOF) { 
	  echo '<div class="dataTableHeadingRow">' . PRICE_SHEETS_LOG . $price_sheets->fields['sheet_name'] . '</div>';
