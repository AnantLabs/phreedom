...
function draw_address_fields($entries, $add_type, $reset_button = false, $delete_button = true, $short = false) {
	...
	if (sizeof($entries->$field_to_find) > 0 && substr($add_type, 1, 1) <> 'm') {
		$field .= '<tr><td><table id="' . $add_type . '_table" class="ui-widget" style="border-collapse:collapse;width:100%;">';
		...
		$field .= '<tbody class="ui-widget-content">' . chr(10) . chr(10);
		
		$odd = true;
		foreach ($entries->$field_to_find as $address) {
			$field .= '<tr id="tr_' . $address['address_id'] . '" class="' . ($odd?'odd':'even') . '" style="cursor:pointer">';
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['primary_name'] ? $address['primary_name'] : '&nbsp;') . '</td>' . chr(10);
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['contact'] ? $address['contact'] : '&nbsp;') . '</td>' . chr(10);
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['address1'] ? $address['address1'] : '&nbsp;') . '</td>' . chr(10);
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['city_town'] ? $address['city_town'] : '&nbsp;') . '</td>' . chr(10);
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['state_province'] ? $address['state_province'] : '&nbsp;') . '</td>' . chr(10);
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['postal_code'] ? $address['postal_code'] : '&nbsp;') . '</td>' . chr(10);
			// add special fields
			$field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['country_code'] ? $address['country_code'] : '&nbsp;') . '</td>' . chr(10);
			if (substr($add_type, 1, 1) == 'p') $field .= '  <td onclick="editRow(\'' . $add_type . '\', ' . $address['address_id'] . ')">' . ($address['hint'] ? $address['hint'] : '&nbsp;') . '</td>' . chr(10);
	...
	if (!$short) {
		$field .= '<tr>';
		$field .= '  <td>' . GEN_PRIMARY_NAME . '</td>' . chr(10);
		$primary_name = $add_type . '_primary_name';
		$field .= '  <td>' . html_input_field($primary_name, $entries->$primary_name, 'size="49" maxlength="48"', true) . '</td>' . chr(10);
		$field .= '  <td align="right">' . GEN_TELEPHONE1 . '</td>' . chr(10);
		$telephone1 = $add_type . '_telephone1';
		$field .= '  <td>' . html_input_field($telephone1, $entries->$telephone1, 'size="21" maxlength="20"', ADDRESS_BOOK_TELEPHONE1_REQUIRED) . '</td>' . chr(10);
		$field .= '</tr>';
	}

	$field .= '<tr>';
	$field .= '  <td align="right">' . GEN_CONTACT . html_hidden_field($address_id, $entries->$address_id) . '</td>' . chr(10);
	$contact = $add_type . '_contact';
	$field .= '  <td>' . html_input_field($contact, $entries->$contact, 'size="33" maxlength="32"', ADDRESS_BOOK_CONTACT_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_TELEPHONE2 . '</td>' . chr(10);
	$telephone2 = $add_type . '_telephone2';
	$field .= '<td>' . html_input_field($telephone2, $entries->$telephone2, 'size="21" maxlength="20"') . '</td>' . chr(10);
	$field .= '</tr>';

	$field .= '<tr>';
	$field .= '<td align="right">' . GEN_ADDRESS1 . '</td>' . chr(10);
	$address1 = $add_type . '_address1';
	$field .= '<td>' . html_input_field($address1 , $entries->$address1, 'size="33" maxlength="32"', ADDRESS_BOOK_ADDRESS1_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_FAX . '</td>' . chr(10);
	$telephone3 = $add_type . '_telephone3';
	$field .= '<td>' . html_input_field($telephone3, $entries->$telephone3, 'size="21" maxlength="20"') . '</td>' . chr(10);
	$field .= '</tr>';

	$field .= '<tr>';
	$field .= '<td align="right">' . GEN_ADDRESS2 . '</td>' . chr(10);
	$address2 = $add_type . '_address2';
	$field .= '<td>' . html_input_field($address2, $entries->$address2, 'size="33" maxlength="32"', ADDRESS_BOOK_ADDRESS2_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_TELEPHONE4 . '</td>' . chr(10);
	$telephone4 = $add_type . '_telephone4';
	$field .= '<td>' . html_input_field($telephone4, $entries->$telephone4, 'size="21" maxlength="20"') . '</td>' . chr(10);
	$field .= '</tr>';

	$field .= '<tr>';
	$field .= '<td align="right">' . GEN_CITY_TOWN . '</td>' . chr(10);
	$city_town = $add_type . '_city_town';
	$field .= '<td>' . html_input_field($city_town, $entries->$city_town, 'size="25" maxlength="24"', ADDRESS_BOOK_CITY_TOWN_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_EMAIL . '</td>' . chr(10);
	$email = $add_type . '_email';
	$field .= '<td>' . html_input_field($email, $entries->$email, 'size="51" maxlength="50"') . '</td>' . chr(10);
	$field .= '</tr>';

	$field .= '<tr>';
	$field .= '<td align="right">' . GEN_STATE_PROVINCE . '</td>' . chr(10);
	$state_province = $add_type . '_state_province';
	$field .= '<td>' . html_input_field($state_province, $entries->$state_province, 'size="25" maxlength="24"', ADDRESS_BOOK_STATE_PROVINCE_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_WEBSITE . '</td>' . chr(10);
	$website = $add_type . '_website';
	$field .= '<td>' . html_input_field($website, $entries->$website, 'size="51" maxlength="50"') . '</td>' . chr(10);
	$field .= '</tr>';

	$field .= '<tr>';
	$field .= '<td align="right">' . GEN_POSTAL_CODE . '</td>' . chr(10);
	$postal_code = $add_type . '_postal_code';
	$field .= '<td>' . html_input_field($postal_code, $entries->$postal_code, 'size="11" maxlength="10"', ADDRESS_BOOK_POSTAL_CODE_REQUIRED) . '</td>' . chr(10);
	$field .= '<td align="right">' . GEN_COUNTRY . '</td>' . chr(10);
	$country_code = $add_type . '_country_code';
	$field .= '<td>' . html_pull_down_menu($country_code, gen_get_countries(), ((substr($add_type, 1, 1) == 'm' && $entries->$country_code <> '') ? $entries->$country_code : COMPANY_COUNTRY)) . '</td>' . chr(10);
	$field .= '</tr>';
	...
	if (substr($add_type, 1, 1) <> 'm' || $add_type == 'im') {
      ...
      $field .= '<td colspan="3">' . html_textarea_field($notes, 80, 5, $entries->$notes, $parameters = '') . chr(10);
