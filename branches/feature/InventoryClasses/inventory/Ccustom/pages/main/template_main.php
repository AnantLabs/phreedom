...
// +-----------------------------------------------------------------+
//  Path: /modules/inventory/custom/pages/main/template_main.php
//
...
<table id="filter_table" class="ui-widget" style="border-collapse:collapse;">
  ...
  <tbody id="filter_table_body" class="ui-widget-content">
	  <?php
	  $x = 1;
	  while (isset($_POST['filter_field'][$x - 1])) {
		echo '<script type="text/javascript"> addFilterRow();</script>' .chr(10);
		echo '<script type="text/javascript"> TableStartValues("' . $x . '","' . $_POST['filter_field'][$x - 1] . '","' . $_POST['filter_criteria'][$x - 1] . '","' . $_POST['filter_value'][$x - 1] . '");</script>'.chr(10);
	     $x++;
	  }
	  if ($x == 1 ) {
	  	echo'<script type="text/javascript"> addFilterRow();</script>'.chr(10);
	  	echo'<script type="text/javascript"> TableStartValues("1","sku","0","");</script>'.chr(10);
	  }
	  ?>	
 </tbody>
 <tfoot>
 	<tr>
		...
		<?php if($x<>0){echo html_icon('actions/view-refresh.png', TEXT_RESET, 'small', 'onclick="location.href = \'index.php?' . gen_get_all_get_params(array('search_text', 'search_period', 'search_date', 'list', 'action')) . '\';" style="cursor:pointer;"');}?>
		</th>
		<th></th>
		<th></th>
 	</tr>
...
<?php // build the action toolbar
	  ...
	  echo html_icon('actions/document-print.png', 'Print Label', 'small', 'onclick="print(\''. $query_result->fields['sku'] .'\', \''.substr($currencies->format_full(($query_result->fields['full_price']* ((100+$extra_tax_rates[$query_result->fields['item_taxable']]['rate']) / 100)), true, DEFAULT_CURRENCY, 1),3) . '\',\''. $query_result->fields['description_sales'] .'\')" ') . chr(10);
