...
class tabs {
  ...
  function build_main_html() {
    ...
    $result = $db->Execute("select id, tab_name, description, sort_order from " . TABLE_EXTRA_TABS . " where module_id='inventory'");
    $rowCnt = 0;
	while (!$result->EOF) {
	  $actions = '';
	  ...
	  if ($_SESSION['admin_security'][SECURITY_ID_CONFIGURATION] > 3) $actions .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . EXTRA_TABS_DELETE_INTRO . '\')) tabPage(\'tabs\', \'delete\', ' . $result->fields['id'] . ')"') . chr(10);
