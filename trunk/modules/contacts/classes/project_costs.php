...
class project_costs {
  ...
  function btn_save($id = '') {
    ...
    if ($id) {
	  db_perform($this->db_table, $sql_data_array, 'update', "cost_id = '" . $id . "'");
...
*/
	// OK to delete
	$result = $db->Execute("select description_short from " . $this->db_table . " where cost_id = '" . $id . "'");
	$db->Execute("delete from " . $this->db_table . " where cost_id = '" . $id . "'");
  ...
  function build_main_html() {
    ...
    $output .= '  </tr>' . chr(10);
	// Build field data
    $query_raw = "select cost_id, description_short, cost_type, inactive 
    ...
    while (!$result->EOF) {
      ...
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'project_costs_edit\', ' . $result->fields['cost_id'] . ')">' . htmlspecialchars($result->fields['description_short']) . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'project_costs_edit\', ' . $result->fields['cost_id'] . ')">' . $project_cost_types[$result->fields['cost_type']] . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" onclick="loadPopUp(\'project_costs_edit\', ' . $result->fields['cost_id'] . ')">' . ($result->fields['inactive'] ? TEXT_YES : '') . '</td>' . chr(10);
      $output .= '    <td class="dataTableContent" align="right">' . chr(10);
	  if ($this->security_id > 1) $output .= html_icon('actions/edit-find-replace.png', TEXT_EDIT,   'small', 'onclick="loadPopUp(\'project_costs_edit\', ' . $result->fields['cost_id'] . ')"') . chr(10);
	  if ($this->security_id > 3) $output .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SETUP_PROJECT_COSTS_DELETE_INTRO . '\')) subjectDelete(\'project_costs\', ' . $result->fields['cost_id'] . ')"') . chr(10);
      $output .= '    </td>' . chr(10);
      $output .= '  </tr>' . chr(10);
      $result->MoveNext();
    }
    $output .= '</table>' . chr(10);
    $output .= '<div class="page_count_right">' . $page_split->display_ajax($query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['list'], '', 'project_costs_list', 'project_costs') . '</div>' . chr(10);
    $output .= '<div class="page_count">'       . $page_split->display_count($query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['list'], SETUP_DISPLAY_NUMBER_OF_PROJECT_COSTS) . '</div>' . chr(10);
	return $output;
  }
  ...
  function build_form_html($action, $id = '') {
    ...
    $sql = "select description_short, description_long, cost_type, inactive 
	    from " . $this->db_table . " where cost_id = '" . $id . "'";
