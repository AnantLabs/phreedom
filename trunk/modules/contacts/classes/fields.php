...
class fields {
  ...
  function btn_save($id = '') {
	...
	$sql = "select id from " . TABLE_EXTRA_FIELDS . " where module_id='contacts' and field_name='" . $field_name . "'";
	...
	if ($id) {
	  ...
	  if (isset($values['entry_type']) || $field_name <> $result->fields['field_name']) {
		$sql = "alter table " . TABLE_CONTACTS . " change " . $result->fields['field_name'] . " " . $field_name . " 
	  ...
	  $sql = "alter table " . TABLE_CONTACTS . " 
  ...
  function btn_delete($id = 0) {
	...
	if ($id <> '0' && $field_name) { // don't allow deletion of system fields
	  ...
	  $db->Execute("alter table " . TABLE_CONTACTS . " drop column " . $field_name);
  ...
  function build_form_html($action, $id = '') {
	...
	// build the tab list
	$tab_list = gen_build_pull_down(xtra_field_get_tabs('contacts'));
