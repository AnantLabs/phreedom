...
// +-----------------------------------------------------------------+
//  Path: /modules/inventory/custom/pages/main/template_tab_bom.php
//
...
<?php
	...
	if (count($bom_list)) {
		for ($j = 0, $i = 1; $j < count($bom_list); $j++, $i++) {
			...
			echo '      <td><input type="text" name="assy_item_cost[]" id="item_cost_' . $i . '" value="' . $bom_list[$j]['item_cost'] . '" ' . $readonly . 'size="6" maxlength="5" /></td>' . chr(10);
			echo '      <td><input type="text" name="assy_sales_price[]" id="sales_price_' . $i . '" value="' . $bom_list[$j]['sales_price'] . '" ' . $readonly . 'size="6" maxlength="5" /></td>' . chr(10);
			echo '    </tr>';
		}
		echo '<script language="JavaScript">updateTotalValues();</script>';
	} else {
...
?>
     ...
     <tfoot>
		  <tr>
			<td><?php if ($cInfo->last_journal_date == '0000-00-00 00:00:00') { // show add button if no posting have been made
