...
//echo 'temp array = '; print_r($temp); echo '<br />';
  if (is_array($temp)) {
	...
	foreach ($temp as $carrier => $value) {
		...
		echo '<th width="10%" rowspan="99" class="dataTableContent" valign="top" align="center">' . constant('MODULE_SHIPPING_' . strtoupper($carrier) . '_TITLE_SHORT') . '<br />' . ($filename ? html_image($filename, $alt = $method, '60', '') : '&nbsp;') . '</th>' . chr(10);
