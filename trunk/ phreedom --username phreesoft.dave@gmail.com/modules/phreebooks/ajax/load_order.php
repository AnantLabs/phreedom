	...
	if ($so_po_ref_id) {	// then there is a purchase order/sales order to load first
	  ...
	  while (!$ordr_items->EOF) {
		$total = $ordr_items->fields['credit_amount'] + $ordr_items->fields['debit_amount'];
		  ...
		  $subtotal   += $total;
		  $inv_details = $db->Execute("select inventory_type, inactive, item_weight, quantity_on_hand, lead_time, purch_package_quantity 
		  ...
		  $item_list[] = array(
			...
			'lead'              => $inv_details->fields['lead_time'],
		  	'purch_package_quantity' => $inv_details->fields['purch_package_quantity'],
		  );
