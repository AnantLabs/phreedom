...
class toolbar {
  ...
  function add_period() {
	...
	$output .= html_pull_down_menu('search_period', gen_get_period_pull_down($this->period_strict), $this->search_period, 'onchange="submitToDo(\'' . gen_get_all_get_params(array('action', 'list')) . ',false\')"');
	$output .= '</div>' . "\n";
	return $output;
  }
...
/**************************************************************************************************************/
class objectInfo {
  function objectInfo($object_array = array()) {
