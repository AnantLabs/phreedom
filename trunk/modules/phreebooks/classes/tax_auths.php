...
class tax_auths {
  ...
  function build_main_html() {
	...
	while (!$result->EOF) {
	  $actions = '';
	  if ($this->security_id > 1) $actions .= html_icon('actions/edit-find-replace.png', TEXT_EDIT, 'small', 'onclick="loadPopUp(\'tax_auths_edit\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
	  if ($this->security_id > 3) $actions .= html_icon('emblems/emblem-unreadable.png', TEXT_DELETE, 'small', 'onclick="if (confirm(\'' . SETUP_TAX_AUTH_DELETE_INTRO . '\')) subjectDelete(\'tax_auths\', ' . $result->fields['tax_auth_id'] . ')"') . chr(10);
	  $content['tbody'][$rowCnt] = array(
	    array('value' => htmlspecialchars($result->fields['description_short']),
			  'params'=> 'style="cursor:pointer" onclick="loadPopUp(\'tax_auths_edit\',\''.$result->fields['tax_auth_id'].'\')"'),
    ...
    return html_datatable('tax_auths_'.$this->type.'_table', $content);
  }
