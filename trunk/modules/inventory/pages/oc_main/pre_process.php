...
// +-----------------------------------------------------------------+
//  Path: /modules/inventory/pages/oc_main/pre_process.php
//
...
$custom_path = DIR_FS_WORKING . 'custom/pages/oc_main/extra_actions.php';
...
switch ($action) {
  ...
  case 'save':
	...
	if (!$error) {
	  ...
	  $category_list = $db->Execute("select id, name, description 
  ...
  case 'properties':
	...
	$category_list = $db->Execute("select id, tab_name, description 
		from " . TABLE_EXTRA_TABS . " where module_id='inventory' order by sort_order");
